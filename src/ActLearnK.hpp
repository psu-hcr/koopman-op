...
class alk {
  ...
  alk(system *_sys, objective *_cost, double _T,const arma::vec& _umax,const arma::mat& _R){
    sys = _sys; cost=_cost; T=_T;umax = _umax;
...
template <class system, class objective>
void alk<system,objective>::ustar_calc(){ 
  arma::vec ustar;
  ustar = arma::zeros<arma::vec>(size(sys->Ucurr));
  arma::mat xsol,rhosol;    
  xsol = xforward();
  rhosol = rhoback(xsol);  
  ustar = -Rinv*(sys->Ku*sys->zfuncs->dvdu(sys->Xcurr)).t()*rhosol.col(0)+cost->mu(sys->Xcurr,sys->tcurr);
  ustar=saturation(ustar);//ustar.u=usched.col(0);
return;}
    
//forward simulation of x
template <class system, class objective>
arma::mat alk<system,objective>::xforward(){
  ...
  for(int i = 0; i<T_index;i++){
    ...
    xsol.col(i)=x0;
    x0 = RK4_step<system,const arma::vec&>(sys,x0,cost->mu(x0,ti),sys->dt);
  }    
return xsol;}

//backward simulaiton of the adjoint
template <class system, class objective>
arma::mat alk<system,objective>::rhoback(const arma::mat& xsol){
  ...
  for(int i = T_index-1; i>=0;i--){
	...
	current.u = cost->mu(current.x,current.t);
